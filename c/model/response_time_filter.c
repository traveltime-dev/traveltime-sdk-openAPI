...
cJSON *response_time_filter_convertToJSON(response_time_filter_t *response_time_filter) {
    ...
    cJSON *itemLocal = response_time_filter_result_convertToJSON(resultsListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
...
response_time_filter_t *response_time_filter_parseFromJSON(cJSON *response_time_filterJSON){
    ...
    response_time_filter_t *response_time_filter_local_var = NULL;

    // response_time_filter->results
    cJSON *results = cJSON_GetObjectItemCaseSensitive(response_time_filterJSON, "results");
    if (!results) {
        goto end;
    }
    ...
    cJSON_ArrayForEach(results_local_nonprimitive,results )
    {
        ...
        response_time_filter_result_t *resultsItem = response_time_filter_result_parseFromJSON(results_local_nonprimitive);

        list_addElement(resultsList, resultsItem);
    }


    response_time_filter_local_var = response_time_filter_create (
        resultsList
        );
