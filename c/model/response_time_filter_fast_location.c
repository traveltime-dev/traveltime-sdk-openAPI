...
void response_time_filter_fast_location_free(response_time_filter_fast_location_t *response_time_filter_fast_location) {
    if(NULL == response_time_filter_fast_location){
        return ;
    }
...
cJSON *response_time_filter_fast_location_convertToJSON(response_time_filter_fast_location_t *response_time_filter_fast_location) {
    ...
    cJSON *itemLocal = response_time_filter_fast_properties_convertToJSON(propertiesListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
...
response_time_filter_fast_location_t *response_time_filter_fast_location_parseFromJSON(cJSON *response_time_filter_fast_locationJSON){
    ...
    response_time_filter_fast_location_t *response_time_filter_fast_location_local_var = NULL;
    ...
    // response_time_filter_fast_location->properties
    cJSON *properties = cJSON_GetObjectItemCaseSensitive(response_time_filter_fast_locationJSON, "properties");
    if (!properties) {
        goto end;
    }
    ...
    cJSON_ArrayForEach(properties_local_nonprimitive,properties )
    {
        ...
        response_time_filter_fast_properties_t *propertiesItem = response_time_filter_fast_properties_parseFromJSON(properties_local_nonprimitive);

        list_addElement(propertiesList, propertiesItem);
    }


    response_time_filter_fast_location_local_var = response_time_filter_fast_location_create (
