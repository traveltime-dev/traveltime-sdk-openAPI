...
, list_t *apiKeys_ApplicationId
    ...
    if(apiKeys_ApplicationId!= NULL) {
        apiClient->apiKeys_ApplicationId = list_create();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_ApplicationId) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_ApplicationId, pairDup);
        }
    }else{
        apiClient->apiKeys_ApplicationId = NULL;
    }
...
void apiClient_free(apiClient_t *apiClient) {
    ...
    if(apiClient->apiKeys_ApiKey) {
        ...
        list_free(apiClient->apiKeys_ApiKey);
    }
    if(apiClient->apiKeys_ApplicationId) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_ApplicationId) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
...
void apiClient_invoke(apiClient_t    *apiClient,
    ...
    if(handle) {
        ...
        // this would only be generated for apiKey authentication
        if (apiClient->apiKeys_ApplicationId != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_ApplicationId) {
        keyValuePair_t *apiKey = listEntry->data;
        if((apiKey->key != NULL) &&
