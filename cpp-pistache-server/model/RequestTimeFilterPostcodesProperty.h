/**
* TravelTime API
* No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
*
* The version of the OpenAPI document: 1.2.3
* Contact: support@igeolise.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * RequestTimeFilterPostcodesProperty.h
 *
 * 
 */

#ifndef RequestTimeFilterPostcodesProperty_H_
#define RequestTimeFilterPostcodesProperty_H_


#include <nlohmann/json.hpp>

namespace org::openapitools::server::model
{

/// <summary>
/// 
/// </summary>
class  RequestTimeFilterPostcodesProperty
{
public:
    RequestTimeFilterPostcodesProperty();
    virtual ~RequestTimeFilterPostcodesProperty() = default;

    enum class eRequestTimeFilterPostcodesProperty {
    // To have a valid default value.
    // Avoiding nameclashes with user defined
    // enum values
    INVALID_VALUE_OPENAPI_GENERATED = 0,
    TRAVEL_TIME, 
    DISTANCE
    };

    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    bool operator==(const RequestTimeFilterPostcodesProperty& rhs) const;
    bool operator!=(const RequestTimeFilterPostcodesProperty& rhs) const;

    /////////////////////////////////////////////
    /// RequestTimeFilterPostcodesProperty members

    RequestTimeFilterPostcodesProperty::eRequestTimeFilterPostcodesProperty getValue() const;
    void setValue(RequestTimeFilterPostcodesProperty::eRequestTimeFilterPostcodesProperty value);

    friend void to_json(nlohmann::json& j, const RequestTimeFilterPostcodesProperty& o);
    friend void from_json(const nlohmann::json& j, RequestTimeFilterPostcodesProperty& o);
protected:
    RequestTimeFilterPostcodesProperty::eRequestTimeFilterPostcodesProperty m_value = RequestTimeFilterPostcodesProperty::eRequestTimeFilterPostcodesProperty::INVALID_VALUE_OPENAPI_GENERATED;

    // Helper overload for validate. Used when one model stores another model and calls it's validate.
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;
};

} // namespace org::openapitools::server::model

#endif /* RequestTimeFilterPostcodesProperty_H_ */
