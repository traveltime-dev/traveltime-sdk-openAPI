...
void ResponseTimeFilterPostcodeSectors::fromJson(const web::json::value& val)
{
    {
        m_Results.clear();
        std::vector<web::json::value> jsonArray;
        for( auto& item : val.at(utility::conversions::to_string_t("results")).as_array() )
        {
            if(item.is_null())
            {
                m_Results.push_back( std::shared_ptr<ResponseTimeFilterPostcodeSectorsResult>(nullptr) );
            }
            else
            {
                std::shared_ptr<ResponseTimeFilterPostcodeSectorsResult> newItem(new ResponseTimeFilterPostcodeSectorsResult());
...
void ResponseTimeFilterPostcodeSectors::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t("."))
    {
