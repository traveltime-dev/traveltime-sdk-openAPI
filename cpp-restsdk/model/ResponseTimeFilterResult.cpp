...
void ResponseTimeFilterResult::fromJson(const web::json::value& val)
{
    setSearchId(ModelBase::stringFromJson(val.at(utility::conversions::to_string_t("search_id"))));
    {
        ...
        for( auto& item : val.at(utility::conversions::to_string_t("locations")).as_array() )
        {
            if(item.is_null())
            {
                m_Locations.push_back( std::shared_ptr<ResponseTimeFilterLocation>(nullptr) );
            }
            else
            {
                std::shared_ptr<ResponseTimeFilterLocation> newItem(new ResponseTimeFilterLocation());
...
void ResponseTimeFilterResult::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t("."))
    {
