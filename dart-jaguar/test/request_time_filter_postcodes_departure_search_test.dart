import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for RequestTimeFilterPostcodesDepartureSearch
void main() {
  var instance = new RequestTimeFilterPostcodesDepartureSearch();

  group('test RequestTimeFilterPostcodesDepartureSearch', () {
    // String id (default value: null)
    test('to test the property `id`', () async {
      // TODO
    });

    // RequestTransportation transportation (default value: null)
    test('to test the property `transportation`', () async {
      // TODO
    });

    // int travelTime (default value: null)
    test('to test the property `travelTime`', () async {
      // TODO
    });

    // DateTime departureTime (default value: null)
    test('to test the property `departureTime`', () async {
      // TODO
    });

    // List<RequestTimeFilterPostcodesProperty> properties (default value: const [])
    test('to test the property `properties`', () async {
      // TODO
    });

    // RequestRangeFull range (default value: null)
    test('to test the property `range`', () async {
      // TODO
    });


  });

}
