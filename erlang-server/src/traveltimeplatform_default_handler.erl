%% basic handler
-module(traveltimeplatform_default_handler).
...
-record(state, {
    operation_id :: traveltimeplatform_api:operation_id(),
    logic_handler :: atom(),
...
-spec init(TransportName :: atom(), Req :: cowboy_req:req(), Opts :: traveltimeplatform_router:init_opts()) ->
    {upgrade, protocol, cowboy_rest, Req :: cowboy_req:req(), Opts :: traveltimeplatform_router:init_opts()}.

init(_Transport, Req, Opts) ->
    {upgrade, protocol, cowboy_rest, Req, Opts}.

-spec rest_init(Req :: cowboy_req:req(), Opts :: traveltimeplatform_router:init_opts()) ->
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'GeocodingReverseSearch'
    }
) ->
    {[<<"GET">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'SupportedLocations'
    }
) ->
    {[<<"POST">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'TimeFilterPostcodeDistricts'
    }
) ->
    {[<<"POST">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'TimeFilterPostcodeSectors'
    }
) ->
    {[<<"POST">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'TimeFilterPostcodes'
    }
) ->
    {[<<"POST">>], Req, State};
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'GeocodingReverseSearch' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
    Result = traveltimeplatform_auth:authorize_api_key(
        LogicHandler,
        OperationID,
        From,
        "X-Api-Key",
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'GeocodingSearch' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'MapInfo' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'Routes' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'SupportedLocations' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'TimeFilter' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'TimeFilterFast' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'TimeFilterPostcodeDistricts' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'TimeFilterPostcodeSectors' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'TimeFilterPostcodes' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
...
is_authorized(
    Req0,
    State = #state{
        operation_id = 'TimeMap' = OperationID,
        logic_handler = LogicHandler
    }
) ->
    From = header,
...
handle_request_json(
    ...
    case traveltimeplatform_api:populate_request(OperationID, Req0, ValidatorState) of
        {ok, Populated, Req1} ->
            {Code, Headers, Body} = traveltimeplatform_logic_handler:handle_request(
                LogicHandler,
                OperationID,
                Populated,
                Context
            ),
            _ = traveltimeplatform_api:validate_response(
                OperationID,
                Code,
                Body,
                ValidatorState
