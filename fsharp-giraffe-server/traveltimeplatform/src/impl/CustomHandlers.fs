...
module CustomHandlers = 
  ...
  let loginView =
    html [] [
        ...
        body [] [
            h1 [] [ str "Welcome" ]
            a [_href "/login-with-ApiKey"] [ str "Login with ApiKey" ]
            a [_href "/login-with-ApplicationId"] [ str "Login with ApplicationId" ]
        ]
    ]
  ...
  let handlers : HttpHandler list = [
    ...
    GET >=> 
      choose [
        ...
        route "/login-with-ApiKey" >=> challenge "ApiKey"
        route "/login-with-ApplicationId" >=> challenge "ApplicationId"
        route "/logout" >=> logout
      ]
  ]
