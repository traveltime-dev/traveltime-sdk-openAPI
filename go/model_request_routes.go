/*
 * TravelTime API
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * API version: 1.2.3
 * Contact: support@igeolise.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// RequestRoutes struct for RequestRoutes
type RequestRoutes struct {
	Locations []RequestLocation `json:"locations"`
	DepartureSearches *[]RequestRoutesDepartureSearch `json:"departure_searches,omitempty"`
	ArrivalSearches *[]RequestRoutesArrivalSearch `json:"arrival_searches,omitempty"`
}

// NewRequestRoutes instantiates a new RequestRoutes object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRequestRoutes(locations []RequestLocation) *RequestRoutes {
	this := RequestRoutes{}
	this.Locations = locations
	return &this
}

// NewRequestRoutesWithDefaults instantiates a new RequestRoutes object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRequestRoutesWithDefaults() *RequestRoutes {
	this := RequestRoutes{}
	return &this
}

// GetLocations returns the Locations field value
func (o *RequestRoutes) GetLocations() []RequestLocation {
	if o == nil {
		var ret []RequestLocation
		return ret
	}

	return o.Locations
}

// GetLocationsOk returns a tuple with the Locations field value
// and a boolean to check if the value has been set.
func (o *RequestRoutes) GetLocationsOk() (*[]RequestLocation, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Locations, true
}

// SetLocations sets field value
func (o *RequestRoutes) SetLocations(v []RequestLocation) {
	o.Locations = v
}

// GetDepartureSearches returns the DepartureSearches field value if set, zero value otherwise.
func (o *RequestRoutes) GetDepartureSearches() []RequestRoutesDepartureSearch {
	if o == nil || o.DepartureSearches == nil {
		var ret []RequestRoutesDepartureSearch
		return ret
	}
	return *o.DepartureSearches
}

// GetDepartureSearchesOk returns a tuple with the DepartureSearches field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RequestRoutes) GetDepartureSearchesOk() (*[]RequestRoutesDepartureSearch, bool) {
	if o == nil || o.DepartureSearches == nil {
		return nil, false
	}
	return o.DepartureSearches, true
}

// HasDepartureSearches returns a boolean if a field has been set.
func (o *RequestRoutes) HasDepartureSearches() bool {
	if o != nil && o.DepartureSearches != nil {
		return true
	}

	return false
}

// SetDepartureSearches gets a reference to the given []RequestRoutesDepartureSearch and assigns it to the DepartureSearches field.
func (o *RequestRoutes) SetDepartureSearches(v []RequestRoutesDepartureSearch) {
	o.DepartureSearches = &v
}

// GetArrivalSearches returns the ArrivalSearches field value if set, zero value otherwise.
func (o *RequestRoutes) GetArrivalSearches() []RequestRoutesArrivalSearch {
	if o == nil || o.ArrivalSearches == nil {
		var ret []RequestRoutesArrivalSearch
		return ret
	}
	return *o.ArrivalSearches
}

// GetArrivalSearchesOk returns a tuple with the ArrivalSearches field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RequestRoutes) GetArrivalSearchesOk() (*[]RequestRoutesArrivalSearch, bool) {
	if o == nil || o.ArrivalSearches == nil {
		return nil, false
	}
	return o.ArrivalSearches, true
}

// HasArrivalSearches returns a boolean if a field has been set.
func (o *RequestRoutes) HasArrivalSearches() bool {
	if o != nil && o.ArrivalSearches != nil {
		return true
	}

	return false
}

// SetArrivalSearches gets a reference to the given []RequestRoutesArrivalSearch and assigns it to the ArrivalSearches field.
func (o *RequestRoutes) SetArrivalSearches(v []RequestRoutesArrivalSearch) {
	o.ArrivalSearches = &v
}

func (o RequestRoutes) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["locations"] = o.Locations
	}
	if o.DepartureSearches != nil {
		toSerialize["departure_searches"] = o.DepartureSearches
	}
	if o.ArrivalSearches != nil {
		toSerialize["arrival_searches"] = o.ArrivalSearches
	}
	return json.Marshal(toSerialize)
}

type NullableRequestRoutes struct {
	value *RequestRoutes
	isSet bool
}

func (v NullableRequestRoutes) Get() *RequestRoutes {
	return v.value
}

func (v *NullableRequestRoutes) Set(val *RequestRoutes) {
	v.value = val
	v.isSet = true
}

func (v NullableRequestRoutes) IsSet() bool {
	return v.isSet
}

func (v *NullableRequestRoutes) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRequestRoutes(val *RequestRoutes) *NullableRequestRoutes {
	return &NullableRequestRoutes{value: val, isSet: true}
}

func (v NullableRequestRoutes) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRequestRoutes) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


